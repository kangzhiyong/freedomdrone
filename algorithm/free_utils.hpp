...
template<typename coordinate_type>
static void bresenham(point<coordinate_type, 2> p0, point<coordinate_type, 2> p1, vector<point<coordinate_type, 2>> &cells)
{
    /*
     ...
     */
    coordinate_type dx = p1[0] - p0[0];
    coordinate_type dy = p1[1] - p0[1];
    
    coordinate_type xsign = dx > 0 ? 1 : -1;
    coordinate_type ysign = dy > 0 ? 1 : -1;
    
    dx = abs(dx);
    dy = abs(dy);
    
    coordinate_type xx, xy, yx, yy;
    if (dx > dy) {
        xx = xsign;
        xy = 0;
        yx = 0;
    ...
    coordinate_type D = 2 * dy - dx;
    coordinate_type y = 0;
    for (int x = 0; x < (dx + 1); x++) {
        cells.push_back({p0[0] + x*xx + y*yx, p0[1] + x*xy + y*yy});
        if (D >= 0) {
            y += 1;
            D -= 2 * dx;
        }
...
template<typename coordinate_type>
static bool collinearity(coordinate_type p1, coordinate_type p2, coordinate_type p3)
{
...
template<typename coordinate_type>
static void prune_path_by_collinearity(vector<coordinate_type> path, vector<coordinate_type> pruned_path)
{
    pruned_path = path;
    int i = 2;
    while (i < pruned_path.size())
    {
        coordinate_type p1 = pruned_path[i - 2];
        coordinate_type p2 = pruned_path[i - 1];
        coordinate_type p3 = pruned_path[i];
...
template<typename coordinate_type>
static void prune_path_by_bresenham(vector<coordinate_type> path, vector<coordinate_type> pruned_path)
{
    pruned_path = path;
    int i = 2;
    while (i < pruned_path.size())
    {
        ...
        # on the next iteration.
        */
        vector<coordinate_type> cells;
        bresenham(p1, p3, cells);
        if (find(cells.begin(), cells.end(), p2))
        {
            /*
            # Something subtle here but we can mutate
            # `pruned_path` freely because the length
