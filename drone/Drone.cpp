...
Drone::Drone(MavlinkConnection *conn)
{
    ...
    _update_property[MessageIDs::STATE] = &Drone::_update_state;
    _update_property[MessageIDs::GLOBAL_POSITION] = &Drone::_update_global_position;
    _update_property[MessageIDs::LOCAL_POSITION] = &Drone::_update_local_position;
    _update_property[MessageIDs::GLOBAL_HOME] = &Drone::_update_global_home;
    _update_property[MessageIDs::LOCAL_VELOCITY] = &Drone::_update_local_velocity;
    _update_property[MessageIDs::RAW_GYROSCOPE] = &Drone::_update_gyro_raw;
    _update_property[MessageIDs::RAW_ACCELEROMETER] = &Drone::_update_acceleration_raw;
    _update_property[MessageIDs::BAROMETER] = &Drone::_update_barometer;
...
void Drone::on_message_receive(MessageIDs msg_name, void *msg1)
{
    ...
    if (msg_name == MessageIDs::CONNECTION_CLOSED)
    {
        stop();
    }
...
void Drone::_update_local_position(V3F p)
{
    _north = p[0];
    _east = p[1];
    _down = p[2];
...
void Drone::register_callback(MessageIDs name, user_callback fn)
{
    /*
...
void Drone::remove_callback(MessageIDs name, user_callback fn)
{
    /*
...
void Drone::notify_callbacks(MessageIDs name)
{
    ...
    iter = _callbacks.find(MessageIDs::ANY);
    if(iter != _callbacks.end())
    {
        auto fns = _callbacks[MessageIDs::ANY];
        for (int i = 0; i < fns.size(); i++) {
            (this->*fns[i])();
        }
    }
...
void Drone::set_home_position(V3F p)
{
    // Set the drone's home position to these coordinates
    try {
        if (m_conn != nullptr) {
