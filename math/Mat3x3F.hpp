...
public:
	...
	static Mat3x3F SkewSymmetric(V3F& w) { return Mat3x3F::SkewSymmetric(w[0], w[1], w[2]); }

	static Mat3x3F OuterProduct(V3F a, V3F b)//a^T*b
	{
		float v[9];
		v[0] = a[0] * b[0];
		v[1] = a[0] * b[1];
	...
	// side = 0 <-> vec x Mat
	// side = 1 <-> Mat x vec
	Mat3x3F Cross(V3F vec, int side)
	{
