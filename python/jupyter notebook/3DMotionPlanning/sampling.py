...
def extract_polygons(data, safe_distance):
    # Generate 2.5D obstacle Polygon with safe_distance
    polygons = []
    ...
    return polygons
class Sampler:
    # Generate random free area nodes
    def __init__(self, data, safe_distance):
        self._polygons = extract_polygons(data, safe_distance)
        ...
        # Create KDTree with the coordinates of the center point of the polygon
        self._tree = KDTree(centers, metric='euclidean')

    def sample(self, num_samples):
        # Generate random coordinate points and use KDTree to extract free area nodes
            ...
            if len(idxs) > 0:
                for ind in idxs: 
                    ...
                    if p.contains(s) and p.height >= s[2]:
                        in_collision = True
            if not in_collision:
                pts.append((int(s[0]), int(s[1]), int(s[2])))
